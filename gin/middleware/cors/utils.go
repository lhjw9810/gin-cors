...
func GeneratePreflightHeaders(c Config, requestHeader []string) http.Header {
	headers := make(http.Header)
	if c.AllowCredentials {
		headers.Set("Access-Control-Allow-Credentials", "true")
	}
	...
	if len(c.AllowHeaders) > 0 {
		preflightHeaders := make([]string, 0)
		allowAnyHeader := allowHeadersAll(c.AllowHeaders)
			...
			if !allowAnyHeader {
				...
				for _, allowHeader := range allowHeaders {
					if strings.EqualFold(h, allowHeader) {
						preflightHeaders = append(preflightHeaders, strings.Trim(allowHeader, " "))
					}
				}
			} else {
				preflightHeaders = append(preflightHeaders, strings.Trim(h, " "))
			}
		}
		value := strings.Join(preflightHeaders, ",")
...
func allowHeadersAll(header []string) bool {
	if len(header) <= 0 {
		return false
	}
